(function() {
  ...
  _.reject = function(collection, test) {
    ...
    function reverse(callbackfn){
      return function(num){
        return !callbackfn(num);
      }
    }
    let result = _.filter(collection,reverse(test))
    return result
  };
  ...
  _.map = function(collection, iterator) {
    ...
    if(Array.isArray(collection)){
      ...
      for (let key in result){
        result[key] = iterator(result[key]);
      }
    }
    return result;
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
		let result = 0;
    let startingPos = accumulator;
    if(accumulator === undefined){
      accumulator = 0;
      result += collection[0];
      startingPos = accumulator +1;
    }
  ...
  console.log('Result:' + result)
    if(Array.isArray(collection)){
      for (let i = startingPos; i < collection.length ; i++){
        // console.log(i,collection[i],iterator(result,collection[i]))
